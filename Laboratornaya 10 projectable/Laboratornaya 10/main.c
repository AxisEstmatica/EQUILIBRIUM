...
void shift(char *ptr, int sme); // shift(указатель на н.массива, сдвиг) || сдвиг влево до размера
...
void shift(char *ptr, int sme) // функция смещения стека, создание
							  // пустых нулевых ячеек слева\либо их удаление
{
	i = 0;
	if (sme == 0)
		return;
