...
void process_line(char buffer[]) // тело функции
{
	...
	found = NO;
	flag = NO;
	C = buffer[0]; // присваеваем первый символ, чтобы не потер€ть его
	j = 0;
	...
	do // цикл с постусловием
	{
		...
		if (c == ' ' || c == '.' || c == ',' || c == '\n' || c == '\0' || c == '\t') // проверка конец слова
		{
			...
			if ((prev_c - 'A' == C - 'A' || prev_c - 'a' == C - 'a' || prev_c - 'a' == C - 'A' || prev_c - 'A' == C - 'a') && found != YES) // проверка на условие
			{
		...
		else
		{
			...
			{
				C = c; // первый символ не разделитель
